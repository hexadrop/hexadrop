...
import { InMemoryEventBus } from '../../../src/infraestructure';
...
const handler4Spy = vi.fn<[unknown], void>(() => undefined);
...
describe('InMemoryEventBus', () => {
	test('should works as expected', async () => {
		...
		const bus = new InMemoryEventBus(handler4);

		bus.subscribe(handler1);

		await bus.publish(event1, event2);

		expect(handler1Spy).toHaveBeenCalledWith({ id: 'id' });
