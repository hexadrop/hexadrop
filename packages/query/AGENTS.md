# @hexadrop/query agent guide

Query pattern utilities with bus and handler registration helpers.

- **Package manager:** Bun (`bun`)
- **Build:** `cd packages/query && bun run prepublishOnly` (runs tsdown)
- **Test:** `bun test packages/query`
